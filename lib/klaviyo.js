...
async function upsertProfile({ email, firstName, utm = {}, source = 'landing_signup' }) {
  const props = { source: source || 'landing_signup', signup_at: new Date().toISOString() };
...
export async function subscribeKlaviyo({ email, firstName, marketing = true, utm = {}, source = 'landing_signup' }) {
  ...
  if (marketing && listId) {
    ...
    try { await upsertProfile({ email, firstName, utm, source }); } catch (err) {
      console.error('[klaviyo] profile upsert before subscribe failed:', err);
    }
    return subscribeToList({ email, listId });
  }
  return upsertProfile({ email, firstName, utm, source });
}
