...
export async function generateHeroImage({ productImageUrls, baseImageUrl }) {
  ...
  const promptText = count === 1
    ? 'The first image is the scene reference. The next image is a product bottle. Replace all bottles in the scene with this one bottle. Preserve the scene\'s composition, lighting, background, shadows, and any other objects exactly as in the reference. Output exactly ONE image in 16:9 landscape format.'
    : `The first image is the scene reference. The next ${count} images are product bottles. Replace all bottles in the scene with these ${count} bottles, arranged naturally. Preserve the scene's composition, lighting, background, shadows, and any other objects exactly as in the reference. Output exactly ONE image in 16:9 landscape format.`;
