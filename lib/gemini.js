...
export async function generateHeroImage({ productImageUrls, baseImageUrl }) {
  ...
  const promptText = count === 1
    ...
    : `Replace the bottles in the first image (the scene) with these ${count} bottles from the following images. Show exactly ${count} bottles arranged naturally. Keep everything else in the scene — composition, lighting, background, shadows, other objects — exactly the same. Output one image in 16:9 landscape format.`;
