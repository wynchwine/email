...
export async function generateHeroImage({ productImageUrls, baseImageUrl }) {
  ...
  const promptText = count === 1
    ? 'Replace the bottles in the first image (the scene) with this bottle from the second image. Show exactly one bottle. Keep everything else in the scene — composition, lighting, background, shadows, other objects — exactly the same. Output one image in 16:9 landscape format.'
    : `Replace the bottles in the first image (the scene) with these ${count} distinct bottles from the following images. Show exactly ${count} bottles — one of each, never repeat or duplicate any bottle. Arrange them naturally. Keep everything else in the scene — composition, lighting, background, shadows, other objects — exactly the same. Output one image in 16:9 landscape format.`;
